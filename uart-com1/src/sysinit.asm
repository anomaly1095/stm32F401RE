...
__wait_sysclk:
  ...
  @ enable USART2 clock (derived from PPRE1 clock running at 42Mhz)
  LDR     r0, =RCC_APB1ENR_BB   @ load the bit-band address for re reg
  ADD     r0, r0, #0x44         @ offset for the bit 11 * 4 bytes
  STR     r1, [r0]

  @ save the link register on the stack and do gpio usart config
  LDR     r0, =GPIOA_BASE
...
@------------------------------------
  ...
  .equ USART_CR1_MASK,0b1001000111111100
